...
const buildTextCanvas = ({
  ...
  let fontSizePx = Math.max(48, Math.min(width * 0.108, height * 0.75));
  const fontFamily = props.fontFamily || computed.fontFamily || "var(--font-goks), sans-serif";
  const fontWeight = props.fontWeight || computed.fontWeight || "400";
  const letterSpacing = -0.055 * fontSizePx;
  const lineHeight = fontSizePx * 1.15;
  const resolvedColor =
    props.color === "currentColor" || !props.color || props.color.startsWith("var(")
      ? computed.color || "#eef1f4"
      : props.color;
  ...
  lines.forEach((line, index) =>
    drawLine(ctx, line, width / 2, startY + index * lineHeight, letterSpacing * fit)
  );
...
export default function WarpText({
  ...
  fontWeight = 400,
  fontFamily = "var(--font-goks), var(--font-grotesk), sans-serif",
  ...
  return (
    ...
    >
      <span
        ...
        className="font-display absolute inset-0 flex items-center justify-center text-center tracking-[-0.055em] transition-opacity duration-300 pointer-events-none"
        style={{
          fontSize: "clamp(3.5rem, 10.5vw, 8.5rem)",
          color,
          opacity: canvasReady ? 0 : 0.8,
        }}
      >
        {text}
      </span>
